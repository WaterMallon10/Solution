...
class Application(tk.Frame):
    ...
    def mouse_left_button_press(self, event):
        ...
        for k in self.keyboard.get_keys():
            if k.key_name.lower() in ('c', 's', 'r', 'u'):
                if (k.key_top_left_x <= event.x <= k.key_top_left_x + k.key_width) and (
                        k.key_top_left_y <= event.y <= k.key_top_left_y + k.key_height):
                    messagebox.showinfo("Key Pressed", k.key_name)
